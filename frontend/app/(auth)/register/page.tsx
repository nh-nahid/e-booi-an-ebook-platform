...
export default function RegisterPage() {
  ...
  const update = (key: keyof RegisterValues, value: string) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (role === "admin" && !values.adminCode.trim()) {
      ...
      >
        ...
        <div className="relative mb-5 flex items-center gap-2">
          <svg viewBox="0 0 100 100" fill="none" className="h-8 w-8">
            <path d="M10 25 L10 75 Q30 65 45 75 L45 25 Q30 20 10 25Z" fill="#2DBDB6" />
            <path d="M90 25 L90 75 Q70 65 55 75 L55 25 Q70 20 90 25Z" fill="#0A0E2A" />
            <path
              d="M40 20 Q42 15 55 15 L75 15 Q75 22 68 22 L48 22 Q45 22 45 26 L45 34 Q42 30 40 30 Z"
              fill="#0A0E2A"
            />
            <path d="M45 34 Q45 30 48 30 L68 30 Q68 37 61 37 L48 37 Z" fill="#0A0E2A" />
            <path d="M45 40 L68 40 Q68 47 61 47 L45 47 Z" fill="#0A0E2A" />
          </svg>
        ...
        <h1 className="relative text-2xl font-bold text-[#0A0E2A]">Create an account</h1>
        <p className="relative mt-1 text-sm text-[#6B7280]">
          Join eBooi and start building your library.
        </p>

        {/* role toggle */}
        <div className="relative mt-5 grid grid-cols-2 gap-1 rounded-full border border-[#E1E5E8] bg-[#F7F9FA] p-1">
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="relative mt-6 space-y-4">
          ...
          <div
            className={`
              ...
              ${role === "admin" ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}
            `}
          >
